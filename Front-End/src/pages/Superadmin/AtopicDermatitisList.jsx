  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        console.log('🔍 Fetching atopic dermatitis records for patientId:', patientId);
        const res = await API.get(`/atopic-dermatitis?patientId=${patientId}`, { headers: { Authorization: `Bearer ${token}` } });
        console.log('📊 Atopic dermatitis API response:', res.data);
        setRecords(res.data);
      } catch (err) {
        console.error('❌ Error fetching atopic dermatitis records:', err);
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-center">
                          <button
                            ...
                            onClick={() => navigate(`/Superadmin/ViewAtopicDermatitis/${patientId}`)}
                          >
