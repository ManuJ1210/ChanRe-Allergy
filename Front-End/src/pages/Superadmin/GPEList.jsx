  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        console.log('🔍 Fetching GPE records for patientId:', patientId);
        const res = await API.get(`/gpe?patientId=${patientId}`, { headers: { Authorization: `Bearer ${token}` } });
        console.log('📊 GPE API response:', res.data);
        setRecords(res.data);
      } catch (err) {
        console.error('❌ Error fetching GPE records:', err);
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-center">
                          <button
                            ...
                            onClick={() => navigate(`/Superadmin/ViewGPE/${patientId}`)}
                          >
