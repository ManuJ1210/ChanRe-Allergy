...
import { fetchAtopicDermatitisList } from '../../features/superadmin/superadminThunks';
...
const ViewAtopicDermatitis = () => {
  ...
  const { atopicDermatitisList, loading, error } = useSelector(state => state.superadmin);

  useEffect(() => {
    if (patientId) {
      dispatch(fetchAtopicDermatitisList(patientId));
    }
  }, [dispatch, patientId]);

  // Get the most recent record or first record if array
  const currentRecord = Array.isArray(atopicDermatitisList) && atopicDermatitisList.length > 0 
    ? atopicDermatitisList[0] 
      ...
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center">
            ...
            <p className="text-slate-600">Loading atopic dermatitis record...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-sm border border-blue-100 p-8">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-slate-600 mb-2">No Atopic Dermatitis Record Found</h3>
              <p className="text-slate-500 mb-6">
                No atopic dermatitis assessment record found for this patient.
              </p>
              <button
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-blue-100 mb-6">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <h3 className="text-sm font-medium text-slate-500 mb-2">Affected Areas</h3>
                <p className="text-slate-900">{currentRecord.affectedAreas || 'N/A'}</p>
              </div>
              <div>
              ...
              <div>
                <h3 className="text-sm font-medium text-slate-500 mb-2">Sleep Disturbance Score</h3>
                <p className="text-slate-900">{currentRecord.sleepDisturbance || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
                <h3 className="text-sm font-medium text-slate-500 mb-2">Dryness Without Eczema</h3>
                <p className="text-slate-900">{currentRecord.drynessWithoutEczema || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
                <h3 className="text-sm font-medium text-slate-500 mb-2">Dryness With Eczema</h3>
                <p className="text-slate-900">{currentRecord.drynessWithEczema || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
                <h3 className="text-sm font-medium text-slate-500 mb-2">Skin Examination</h3>
                <p className="text-slate-900">{currentRecord.skinExamination || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
                <h3 className="text-sm font-medium text-slate-500 mb-2">Local Applications</h3>
                <p className="text-slate-900">{currentRecord.localApplications || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
        ...
        {currentRecord.intensity && (
          <div className="bg-white rounded-xl shadow-sm border border-blue-100 mb-6">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {Object.entries(currentRecord.intensity).map(([key, value]) => (
                  <div key={key}>
