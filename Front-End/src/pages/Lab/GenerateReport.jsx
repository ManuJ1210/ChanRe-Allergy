...
const GenerateReport = () => {
  ...
  const fetchTestRequestDetails = async () => {
    try {
      ...
      if (response.data.reportNotes) {
        setFormData(prev => ({
          ...prev,
          reportNotes: response.data.reportNotes || '',
  ...
  const generatePDFReport = async () => {
    try {
      ...
      // In a real implementation, you would send this to backend to generate PDF
...
Generated on: ${new Date().toLocaleString()}
      ...
      const file = new File([blob], `test-report-${testRequest._id}.txt`, { type: 'text/plain' });
      setReportFile(file);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If no file is selected, generate one
      if (!fileToUpload) {
        fileToUpload = await generatePDFReport();
      }

      // Create FormData for file upload
      const formDataToSend = new FormData();
      formDataToSend.append('reportFile', fileToUpload);
      formDataToSend.append('reportNotes', formData.reportNotes);
      formDataToSend.append('reportSummary', formData.reportSummary);
      formDataToSend.append('clinicalInterpretation', formData.clinicalInterpretation);
      formDataToSend.append('qualityControl', formData.qualityControl);
      formDataToSend.append('methodUsed', formData.methodUsed);
      formDataToSend.append('equipmentUsed', formData.equipmentUsed);
      formDataToSend.append('reportGeneratedDate', new Date().toISOString());
      formDataToSend.append('status', 'Report_Generated');

      const response = await API.put(`/test-requests/${id}/generate-report`, formDataToSend, {
        headers: {
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Create and Upload Laboratory Test Report</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-blue-700">{reportFile.name}</span>
                        </div>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex items-center px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {saving ? (
                    <>
